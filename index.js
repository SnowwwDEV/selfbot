...
bot.on("message", async message => {
  ...
  let command = args.shift().toLowerCase();
    ...
    if(command === "hastebin" || command === "haste"){
         ...
         if(!contenu){
             if(config.typeEnvoieMessage === "console"){ return console.log(red("[WARN]") + white(" Contenu invalide. Il est impossible de crée un hastebin vide.")) } else { if(config.typeEnvoieMessage === "salon"){ return message.channel.send("[ERROR] Contenu invalide. Il est impossible de crée un hastebin vide.").catch(err => console.log(red("[ERROR]") + white(" Une erreur inconnue s'est produite lors de l'envoie d'une réponse."))) }} // Au cas où on ne met rien
         } else {
         }
